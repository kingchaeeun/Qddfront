...
import { ScoreTriplet } from './ScoreTriplet';
...
const koreanTranslations: { [key: number]: string } = {
  ...
  2: '전직 대통령은 한국 협상의 어려움을 언급했습니다. "우리가 해결해야 할 문제들이 있었지만, 합의에 도달할 수 있다고 믿습니다"라고 트럼프는 기자들에게 말했습니다. 그의 발언은 무역 관계자들과��� 2시간 회의 후에 나왔습니다.',
...
/**
 * CandidateCard: 원문 후보 단일 카드
 */
export function CandidateCard({ source, onViewSource }: CandidateCardProps) {
  ...
  const getSimilarityBarColor = (score: number) => {
    if (score >= 80) return 'bg-similarity-high';
    if (score >= 70) return 'bg-similarity-medium';
    return 'bg-similarity-low';
  };

  const getDistortionBarColor = (score: number) => {
    if (score >= 70) return 'bg-distortion-high';
    if (score >= 50) return 'bg-distortion-medium';
    return 'bg-similarity-high';
  };

  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-md">
      {/* Card Title */}
      <h3 className="mb-3 text-gray-800 text-lg">{source.title}</h3>
      ...
      <a
        ...
        className="flex items-center gap-1 text-primary-blue underline mb-4 hover:text-blue-700 transition-colors"
      >
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-gray-700 text-sm">유사도 점수 (신뢰도)</span>
          <span className={getSimilarityColor(source.similarityScore)}>
            {source.similarityScore}/100
          </span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div
            className={`h-2 rounded-full ${getSimilarityBarColor(source.similarityScore)}`}
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-gray-700 text-sm">왜곡 점수</span>
          <ScoreIndicator score={source.distortionScore} />
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div
            className={`h-2 rounded-full ${getDistortionBarColor(source.distortionScore)}`}
            style={{ width: `${source.distortionScore}%` }}
          />
        </div>
      </div>

      {/* Detail Scores with Radar Chart */}
      <div className="mb-5">
        <h4 className="text-gray-700 mb-3 text-sm">세부 점수 분석</h4>
        <div className="flex items-start gap-2">
          <div className="flex-1">
            <ScoreTriplet
              semanticReduction={source.scores.semanticReduction}
              interpretiveExtension={source.scores.interpretiveExtension}
              lexicalColoring={source.scores.lexicalColoring}
            />
          </div>
      ...
      <button
        ...
        className="w-full py-3 bg-primary-blue text-white rounded-xl hover:bg-blue-700 transition-colors"
      >
        원문 보기
      </button>
    </div>
